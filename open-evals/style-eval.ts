...
import { gradeStyle } from "./lib/graders";
import { readTextFile, writeJsonFile } from "./lib/io";
import type { ResearchEvalCase } from "./lib/types";

function printHelp() {
  console.log(`Usage:
  npx tsx open-evals/style-eval.ts --query "<query>" --result result.md [--out style.json]
...
async function main() {
  ...
  const grade = await gradeStyle({ caseSpec, finalMarkdown });
