...
每个 session 有自己的 `workspaceRoot`。用户创建 session 时选择目录，后续 `/api/runs` 请求都会携带这个路径。

服务端在 `POST /api/runs` 中校验目录存在且是目录。工具层通过 `resolveWorkspacePath` 把相对路径解析到 workspace 内，并拒绝越界路径。
