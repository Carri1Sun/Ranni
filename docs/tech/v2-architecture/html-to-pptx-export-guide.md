...
- PPTX 自定义布局使用精确的 `1280 / 96 × 720 / 96` 英寸，输出 `12192000 × 6858000 EMU`，避免横向尺寸截断。
...
- 原子文本使用叶子节点和 `white-space: nowrap; word-break: keep-all; overflow-wrap: normal;`，文本框至少预留 `5%`、不低于 `8px` 的宽度余量。
- 中文 deck 按实际字符从 `@fontsource-variable/noto-sans-sc` 选择 WOFF2 子集，在 Chromium 中用同一字体测量，并把合并后的字体写入 PPTX。
...
7. 在 `measurements.json` 记录原子文本宽度余量、行数、稳定化候选和 Chromium 实际字体。
8. 导出前把 workspace 内本地 `<img>` 内联为 data URI，加载并写回实际字体，并对零余量原子文本增加 `nowrap` 与宽度缓冲。
9. 将稳定化后的 `deck.prepared.html` DOM 交给 `dom-to-pptx`。
...
| 字体缺失 | 用户机器打开后变形 | 中文 deck 嵌入 Noto Sans SC 字符子集，并在 QA 中检查嵌入关系 |
...
- `validate_html_pptx_export` 会校验精确 slide EMU、Chromium 实际 CJK 字体、PPTX 字体声明、嵌入字体状态以及原子文本 `wrap=none` 覆盖率。
- macOS headless LibreOffice 渲染依赖 Homebrew 运行库；当前验证环境已补齐 `little-cms2` 和 `fontconfig`。

后续产品化计划：

1. 统计 PPTX 中可编辑文本与源 HTML 标注的逐节点对应关系。
2. 为稳定 HTML 子集沉淀自研 mapper，继续保留局部截图回退策略。
