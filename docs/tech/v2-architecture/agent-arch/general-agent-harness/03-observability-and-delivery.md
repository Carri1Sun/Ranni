...
| 范围 | 当前实现 |
| --- | --- |
| durable events | 已增加 Tool Batch / Receipt、Observed State、Attempt、Acceptance、Progress、Recovery 和 Completion 事件 |
| Run Trace | `RunTraceStore` 将脱敏 Event Log、Run summary、Step index 和逐 Step I/O 写入 Session workspace |
| 输入冻结 | 首次 `context.snapshot` 与 `model.request` 保存 Context 和 exact request，并生成持久化 input snapshot hash |
| 输出聚合 | 聚合 thinking、assistant text、tool call/result/receipt、TaskState、Observed、Attempt、Assumption、Acceptance、Progress、Completion 与 Recovery |
| 查询 API | 已提供 Session Run 列表、Run Step 索引、单 Step I/O 三个接口 |
| EventMapper | 默认采用确定性映射；辅助模型改写需要显式环境变量启用 |
| 产品 UI | 运行概览、验收清单、交付缺口、完成依据、上下文健康和 Step 输入输出查看器已消费持久化 API |

运行中的查询使用 RunRegistry workspace 映射；历史 Session 由 UI 附带已选择的 workspaceRoot，后端据此发现进程重启前的 Run 和 Step I/O。独立 Raw / Diff / 区间导出 API、cursor / filter、checkpoint resume 和完整运行控制区继续作为后续工作。

首批 UI 已包含运行概览、当前路线、下一步、验收清单、交付缺口、当前阻塞、完成依据、进展回执，以及 Step 的 Input / Output / 原始数据、输入构成、上下文健康和工具配对。Step 对比、筛选搜索、长列表虚拟化、暂停与 checkpoint 恢复按钮仍属于本章后续设计。
...
`assumption.invalidated` 只在相同策略连续产生真实失败回执，或 `noMeaningfulProgressStreak` 达到路线重置阈值时发布，记录 assumptionIds、reason 和 evidenceRefs。`noObjectiveProgressStreak` 的 3 / 6 轮阈值只发布交付节奏提醒。
...
前端先加载 Step 索引，选中后按需加载详细 I/O。运行中的 Step 继续通过 SSE 增量更新。Session Run 列表可以带 `workspaceRoot`，合并当前进程 Run 与磁盘中匹配 sessionId 的历史 Run。
...
当前运行概览已经展示路线、验收、交付缺口、阻塞、下一步、完成依据和进展回执。下图中的暂停与 checkpoint 恢复属于后续完整控制区设计：
...
- 查看完成依据和进入 Step 详情的入口。

运行中的补充消息继续通过现有 Steering 通道提交。暂停、checkpoint 自动恢复和完整运行控制区作为后续增强；接入后需要沿用现有授权边界，并在恢复时创建新 attempt、保留原 Event Log。完整分支树和并行路线比较继续根据真实运行数据评估。
...
| `lib/runs/run-trace-store.ts` | 脱敏 Event Log、Run summary、Step index 和逐 Step I/O 持久化 |
| `src/server/run-trace-routes.ts` | Session Run 列表、Step 索引和单 Step I/O 查询 API |
| `src/server/app.ts` | 初始化 Trace Store，并在启动 Run 前建立持久化目录 |
| `components/agent-console.tsx` | 持久化查询、运行详情装配、重启后历史发现和运行状态栏入口 |
| `components/run-observability.tsx` | 运行概览、Step 输入输出查看器、Context 健康、输入输出与 Raw tabs |
| `components/run-observability-model.ts` | Acceptance / Progress / Attempt 投影、工具配对、因果健康和 Legacy fallback |
| `components/agent-console.module.css` | 语义卡片、验收状态、输入输出 tabs、Context health 和窄屏布局 |
...
7. 实现同策略真实失败 2 轮检查、`noObjectiveProgressStreak` 3 / 6 轮提醒、`noMeaningfulProgressStreak` 6 轮路线重置与 10 轮 checkpoint。
8. 将语义失效投影与容量压缩分离。
...
本批的持久化、查询 API、运行概览、Step I/O、上下文健康、按需加载和 Legacy fallback 已完成。完整运行控制、Step Diff、独立导出、筛选搜索与虚拟列表作为后续增强。
...
4. 增加 Steering；暂停和 checkpoint 恢复入口进入后续增强。
5. 重构运行详情页。
...
后续增强验收包括 checkpoint 恢复创建新 attempt、260 个以上 Step 的虚拟列表性能，以及 Step / 区间独立导出。
...
checkpoint 自动恢复的集成测试随恢复入口在后续增强中补充。
