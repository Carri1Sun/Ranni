...
本文件负责可执行 Runtime 的内部设计。Task Contract、Observed State、Working Set、Tool Receipt、Progress Receipt、Deliverable Contract、Working Plan / Plan Ledger、Attempt / PlanAttemptLedger 和 Acceptance Ledger 的语义引用《总览与共享契约》。Event Log 持久化、查询 API、运行概览和 Step 查看器由《可观测性与交付》负责。
...
- Plan Ledger 已通过 `update_plan` 维护稳定 Plan Item、Plan Revision 和 Plan Focus，并根据 Tool Receipt、Acceptance 和 Finalization 生成 Objective Projection。`TaskState.plan` 保留为兼容投影。
- Acceptance、Progress、PlanAttemptLedger、`replace_attempt`、同策略真实失败检查、`noObjectiveProgressStreak` 3 / 6 轮提醒、`noMeaningfulProgressStreak` 6 轮路线重置与 10 轮 checkpoint 已接入每个工具 Step。
...
- ChatGPT Subscription Provider 已实现原子 SSE 提交与最多两次额外瞬时故障重试。`runAgentTurn` 可以通过 `recoveryState` 重建中断现场，新 Steering 追加到恢复后 Conversation，已完成工具调用由 Receipt Registry 复用。

后续结构增强包括继续缩小 Step Runner、接入产品级 checkpoint 选择与恢复控件，以及在现有 workspace、命令黑名单与工具级防线之上抽象通用 SideEffectGate。这些增强不影响本次核心 Runtime 完成状态。长回答 chunked-final 已迁入独立纯控制器，并在聚合完成后进入 Acceptance 验收。

当前 Composition Manifest 已记录 stable prefix hash、真实失效原因、cache-eligible message count、Skill version / body hash、压缩原因、因果配对和 snapshot hash。Research Handoff 提供 thesis、findings、claimIds、sourceIds、artifactPlan、openGaps 和 weakEvidence。模型 assumptions 会进入 PlanAttemptLedger；具有相同策略签名的真实失败连续发生，或 `noMeaningfulProgressStreak` 达到路线重置阈值时，Harness 才会把相关假设标记为 rejected 并发布 `assumption.invalidated`。`noObjectiveProgressStreak` 的 3 / 6 轮阈值只提供交付节奏提醒。
...
| Working Set | 当前意图、Observed State、Working Plan、Plan Focus、Active Attempt、有效假设、Acceptance Gap、artifact、失败、handoff | 每轮重建 | 最高 |
...
2. PlanAttemptLedger 更新 Attempt、假设、证据引用和替代关系。
3. Plan Ledger 根据新回执和 Acceptance 生成 Objective Projection，保留 Working Plan 的稳定计划项。
4. Working Set 在下一轮停止呈现已经失效的假设。
5. Recent Causal Tail 继续完整保留刚发生的证伪过程。
6. Archive Summary 在下次重建时记录结论和引用，避免重新采用同一路线。
...
2. 将旧工具正文替换为摘要和内容引用。
3. 合并已经完成的旧阶段。
4. 压缩已解决问题和较早 Plan Revision 细节，保留当前 Working Plan。
5. 保留最近 2–4 个完整 Causal Turn。

Task Contract、Working Set、Working Plan、上一轮完整 Causal Turn、当前未解决错误和当前 artifact 始终保留。
...
模型通过 `update_plan` 提出工作覆盖、顺序与焦点修订。Harness 保存稳定 Plan Item ID，并根据 Tool Receipt、Acceptance Snapshot 与 Finalization 生成 Objective Projection：
...
type PlanItemRecord = {
  ...
  title: string;
  intent: string;
  status: "pending" | "active" | "blocked" | "satisfied" | "cancelled" | "superseded";
  modelStatus: "pending" | "in_progress" | "completed" | "blocked" | "cancelled";
  acceptanceRefs: string[];
  attemptIds: string[];
  evidenceRefs: string[];
  dependsOn: string[];
  statusSource: "model" | "receipt" | "acceptance" | "finalization";
};
...
Plan Revision 保存修订原因、变更计划项、Step 和可选 Observed State hash。Objective Projection 只更新客观状态与证据，使用独立 `projectionVersion`。计划项的 `completed` 模型报告需要回执、Acceptance 或 Finalization 依据支持，才会投影为 `satisfied`。

Plan Ledger 使用局部 ID 计数器预演整个候选计划，随后校验规范化 ID 和依赖图。重复 ID、自依赖、未知依赖或依赖环会使本次 `update_plan` 失败；校验通过后才一次提交 Plan Snapshot 与 ID 计数器，避免失败输入留下部分修订。

#### Attempt / PlanAttemptLedger

模型在具体方法、关键假设或退出条件实质变化时通过 `replace_attempt` 提出新 Attempt。Harness 负责保存结构、关联 Tool Receipt，并根据验证和错误事实更新结果：
...
该工具更新 Agent Note。兼容 `plan` 字段会经 `PlanLedger.updateLegacy` 桥接为 Working Plan 修订，它只保留标题列表并尽量复用稳定 Plan Item ID。`update_task_state` 不创建 Attempt，也不能直接把 Acceptance Criterion 标记为 passed 或声明 objective progress。

#### `update_plan`

提交当前完整 Plan Item 集合、修订原因和可选 Plan Focus。修订已有计划项时复用当前 Working Plan 中的稳定 ID。工具允许模型报告 `completed`，Harness 仍根据客观依据决定 `satisfied`。

#### `replace_attempt`

提交新的具体 approach、路线替代原因、关键假设和退出条件。PlanAttemptLedger 结束当前 Attempt，创建并激活新 Attempt，同时保留替代关系。计划项措辞、顺序或常规下一动作变化使用 `update_plan` 或 Agent Note。

`update_task_state`、`update_plan` 和 `replace_attempt` 都属于协调状态工具。它们的成功回执不增加 objective progress 或 information gain，也不更改策略签名。
...
- 同义改写 Agent Note。
- 只提交 Plan Revision、Plan Focus 或 Attempt 协调变更。
- 重复读取未变化文件。
- 相同搜索返回相同结果。
- 重复报告同一个错误。
...
当 `noMeaningfulProgressStreak` 达到路线重置阈值，或相同策略连续产生真实失败回执时，PlanAttemptLedger 才会记录 failed、superseded 和 assumption invalidation。Plan Ledger 在同一 Step 保持计划项，后续由模型使用 `update_plan` 调整覆盖或 Plan Focus。路线失效判断绑定 Tool Receipt 与状态 hash。
...
2. 保存 Agent Note、Observed State、Working Plan、PlanAttemptLedger、Progress Tracker、Causal Tail 和 artifact receipts。
3. 执行少量有界重试和退避。
4. 检查 Deliverable Contract、Acceptance Ledger、Working Plan 和当前 Attempt。
5. 交付已完成时进入 final recovery。
6. 交付仍有缺口时恢复执行或返回可恢复 checkpoint。

恢复流程不能清空已经建立的 Causal Tail，也不能在工件 pending 时禁止工具调用。

当前 `AgentRunRecoverySnapshot` 使用 schema version 2，序列化 Conversation、Task Contract、TaskState、Observed State、Acceptance、Plan Ledger、PlanAttemptLedger、Progress Tracker、Active Skills、Plan Authority、stable prefix、chunked-final 状态，以及 Session / workspace 恢复绑定。兼容读取缺少 `planAuthority` 的旧快照时采用 `legacy`，Revision 编号不参与权威来源推断。Task Memory 在 checkpoint Markdown 旁边原子写入权限为 `0600` 的同名 JSON。后续调用把该快照作为 `RunAgentTurnOptions.recoveryState` 传入，Run Controller 校验绑定并通过 `restoreAgentRunState` 重建现场，`run.started.resumedFromCheckpoint` 记录原已完成 Step、Context snapshot hash 和 Plan Revision。Server 会把失败 Run 的可恢复快照一次性交给同 Session、同 workspace 的下一 Run。
...
└── Run Controller
    ...
    ├── Receipt Registry
    ├── Plan、Progress、Attempt、Acceptance
    └── Recovery Controller
...
Finalization Controller 负责 Guard 顺序并返回 `guard_retry` 或 `final`。完成检查通过后，Step Runner 用完成依据终结 Working Plan 和当前 Attempt。Recovery Controller 读取 Deliverable Contract、Acceptance Ledger、Working Plan 和当前 Attempt 后返回 `recover` 或 `failed`。外层循环不再通过十余处 `continue` 表达业务状态。
...
| `lib/task-state.ts` | Task Contract、Agent Note、Observed State 的兼容与迁移；`TaskState.plan` 是 Working Plan 的兼容投影 |
| `lib/plan.ts` | Working Plan、Plan Item、Plan Revision、Objective Projection、Plan Focus 与恢复序列化 |
| `lib/plan-attempt.ts` | Attempt、Assumption、失败阈值消费、失效和替代关系 |
| `lib/acceptance.ts` | Acceptance Ledger、交付缺口、证据绑定和用户豁免 |
| `lib/task-memory.ts` | `plan.json`、Plan Item `todo.md` 投影、checkpoint、错误、证据和 durable memory；Working Set 由 Run State 构建 |
| `lib/tools.ts` | 保留工具公共 registry facade，注册 `update_plan` 和 `replace_attempt`；定义和执行逐步迁入 `lib/tools/` 子模块 |
